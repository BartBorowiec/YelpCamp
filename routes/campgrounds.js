...
router.put("/:slug", middleware.checkCampgroundOwnership, function(req, res){
  geocoder.geocode(req.body.campground.location, function (err, data) {
    ...
    req.body.campground.slug = slugify(req.body.campground.name);
    
    Campground.findOneAndUpdate({slug: req.params.slug}, req.body.campground, function(err, campground){
        if(err){
            req.flash("error", err.message);
            res.redirect("back");
        } else {
            req.flash("success","Successfully Updated!");
            res.redirect("/campgrounds/" + req.body.campground.slug);
        }
    });
  });
});
